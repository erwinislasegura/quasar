...
final class AdminModuleRepository
{
    ...
    public function rows(string $module): array
    {
        ...
        $sql = match ($module) {
            ...
            'archivos' => 'SELECT a.id AS ID, a.nombre AS Archivo, e.nombre AS Equipo, a.estado AS Estado, DATE_FORMAT(a.created_at,"%d-%m-%Y %H:%i:%s") AS Fecha FROM archivos a JOIN equipos e ON e.id=a.equipo_id ORDER BY a.created_at DESC',
            ...
            'auditoria' => 'SELECT a.id AS ID, COALESCE(u.nombre,"Sistema") AS Usuario, a.accion AS Acción, DATE_FORMAT(a.created_at,"%d-%m-%Y %H:%i:%s") AS Fecha FROM auditoria a LEFT JOIN usuarios u ON u.id=a.usuario_id ORDER BY a.created_at DESC',
            default => null,
        };
